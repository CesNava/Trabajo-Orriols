...
## FORMATO DE DATOS: AÑADIMOS LA BASE DE DATOS CON SERIES TEMPORALES PARA PONER LOS DATOS ANUALIZADOS

cargamos datos

```{r}
df_ts <- read_dta("qog_std_ts_jan23_stata14.dta")
...
library(tidyr)
df_ts <- df_ts |> 
 pivot_wider(names_from = year, values_from = glob_index) # pasamos a país como unidad de observación
...
df_models <- df_models |> 
 mutate(estabilidad = as.numeric(estabilidad)) |> 
 mutate(demo = as.numeric(demo)) |> 
 mutate(idh = as.numeric(idh))

descriptivos_table <- sapply(df_models, function(x) c(Mínimo = min(x), Máximo = max(x), Media = mean(x), Mediana = median(x), SD = sd(x)))
